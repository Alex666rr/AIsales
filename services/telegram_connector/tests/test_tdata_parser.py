...
def _account_file_name(index: int) -> str:
    ...
    return "".join(f"{byte & 0x0f:X}{byte >> 4:X}" for byte in digest)


def _snapshot_with_key_data(tmp_path: Path, *, passcode: str = "desktop-passcode"):
    salt = b"s" * 32
    local_key = b"l" * 256
    ...
    (source / f"{_account_file_name(0)}s").write_bytes(_tdf(_qbytearrays(_encrypt_descriptor(account_payload, local_key))))
