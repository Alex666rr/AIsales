...
**Статус:** финальная редакция ТЗ для оценки и планирования разработки  
...
- хранить историю, память и независимые статусы каждого клиента независимо от Telegram-аккаунта;
...
4. объём четырёх последовательных этапов выпуска: от технического прототипа до оптимизации.
...
MVP считается полезным, если платформа позволяет провести пилотную кампанию полным циклом: импортировать контакты, распределить их между аккаунтами, отправить цепочку с заданными ограничениями, получить ответы, провести разрешённые AI-диалоги, передать исключения менеджерам, сохранить результаты и выгрузить их в Google Sheets без потери истории и повторных отправок. До прохождения контрольного барьера Telegram/AI полный цикл проверяется без AI на реальных переписках и с AI только на разрешённых тестовых данных.
...
- Юридические требования конкретной страны не определяются этим документом. При этом контрольный барьер использования Telegram и AI, описанный ниже, является обязательным условием запуска. Технические возможности хранения, удаления, экспорта, контроля доступа и аудита реализуются независимо от последующего юридического решения.

### 3.3. Обязательный контрольный барьер Telegram/AI

- Подключение пользовательского аккаунта по номеру, QR, TData или session использует Telegram API/MTProto от имени этого аккаунта; формат авторизации не исключает применение условий Telegram API.
- Telegram-боты подключаются отдельно через Bot API и дополнительно учитывают условия Telegram Bot Platform.
- AI не получает сообщения из реальных Telegram-диалогов и не формирует для них автоматические ответы, пока владелец платформы не зарегистрировал утверждённую допустимую схему использования для каждого типа канала.
- Решение фиксируется отдельной записью: дата проверки, проверенная редакция условий, пользовательские аккаунты и/или боты, допустимые данные и операции, ограничения, ответственный и подтверждающие материалы.
- Сам факт технического доступа или успешного прототипа не является подтверждением допустимости. Основание решения должно учитывать действующие условия Telegram, оценку применимого сценария и, при необходимости, отдельное юридическое заключение или разъяснение Telegram.
- До подтверждения разрешены Telegram-прототип без AI, ручная работа менеджеров и AI-тесты только на синтетических либо созданных вне Telegram тестовых данных.
- Если допустимая схема не подтверждена, AI-интеграция для реальных Telegram-диалогов остаётся технически заблокированной; архитектура или сценарий использования пересматриваются без попыток обхода ограничений.
...
- Меняет разрешённые этапы продажи, результаты кампании и примечания.
...
**TG-019.** Серверный контрольный барьер по умолчанию запрещает доступ AI к сообщениям реальных Telegram-диалогов и автоматическую AI-отправку.  
**TG-020.** Барьер можно открыть только после регистрации утверждённого решения из раздела 3.3 и только для перечисленных в нём компаний, типов каналов, данных и операций.  
**TG-021.** Пока барьер не пройден, реальные Telegram-диалоги обрабатываются менеджерами вручную, а AI проверяется только на разрешённых тестовых данных, не полученных из Telegram.  
**TG-022.** Изменение способа авторизации, импорт готовой сессии или переход между пользовательским аккаунтом и ботом не отменяют контрольный барьер и требуют отдельной проверки применимых условий.
...
**CNT-011.** Контакт с разрешением на контакт `Стоп-лист` можно включить в список для аналитики, но постановка в очередь требует отдельного снятия запрета владельцем или администратором.  
...
**CNT-017.** Другие массовые действия: добавление в кампанию, назначение тегов, смена менеджера, экспорт, архивирование и разрешённое изменение статусных измерений.

### 6.5. Статусная модель, стоп-листы и блокировки

**STS-001.** Вместо одного общего статуса система хранит три независимых измерения:

- этап продажи: `Новый`, `Сообщение отправлено`, `Ответил`, `Квалифицирован`, `Оффер отправлен`, `Передан менеджеру`, `Отложено`, `Успешно`, `Отказ`;
- разрешение на контакт: `Можно писать`, `Стоп-лист`;
- техническое состояние контакта: `Доступен`, `Предполагаемый блок`, `Подтверждённый блок`, `Невалидный контакт`, `Техническая ошибка`.

**STS-002.** Изменение одного измерения не перезаписывает остальные. Например, клиент может сохранить этап `Квалифицирован`, одновременно иметь разрешение `Стоп-лист` и техническое состояние `Подтверждённый блок`.  
**STS-003.** Результат участия контакта хранится отдельно для каждой кампании. `Игнор` является результатом конкретной кампании и устанавливается после завершения настроенной цепочки без ответа; он не заменяет три текущих измерения карточки клиента.  
**STS-004.** Только однозначная просьба больше не писать или другой явный отзыв разрешения на контакт переводит клиента в глобальный стоп-лист компании.  
**STS-005.** Обычный отказ от предложения, включая негативную реакцию без запрета дальнейшего контакта, меняет этап продажи на `Отказ`, но сам по себе не устанавливает `Стоп-лист`.  
**STS-006.** Косвенные технические признаки дают только состояние `Предполагаемый блок`.  
**STS-007.** `Подтверждённый блок` устанавливается по однозначному техническому сигналу либо вручную менеджером с обязательным примечанием.  
**STS-008.** Для каждого изменения сохраняются измерение, старое и новое значения, кампания, аккаунт, номер касания, дата, причина, комментарий, автор и неизменяемая история.  
**STS-009.** Пример примечания: `Заблокировал аккаунт @sales_1 после второго сообщения кампании Август`.  
**STS-010.** Этап `Успешно` поступает из CRM/партнёрской системы через webhook либо подтверждается менеджером. AI может предложить только сигнал `Готов к покупке`.  
**STS-011.** Отдельные списки в интерфейсе являются фильтрами единой модели данных, а не независимыми копиями клиента.
...
**INB-002.** Фильтры: компания, кампания, аккаунт, этап продажи, результат кампании, разрешение на контакт, техническое состояние, причина передачи, менеджер, AI-профиль и период.  
**INB-003.** Карточка содержит переписку, идентификаторы, кампанию, аккаунт, три текущих статусных измерения, результаты кампаний, память, оффер, причину передачи, источники, примечания и историю изменений.  
...
- уровень уверенности;
- рекомендуемый переход этапа продажи;
- рекомендуемое действие по разрешению на контакт с причиной; AI может предложить установку `Стоп-лист`, но не его снятие;
- рекомендуемый результат текущей кампании;
...
**AID-016.** При неутверждённом контрольном барьере backend не передаёт AI содержимое реальных Telegram-сообщений и отклоняет любую автоматическую AI-отправку независимо от настроек кампании.
...
**GSH-004.** Основной реестр содержит ID, Telegram ID, username, телефон, имя, кампанию, аккаунт, номер касания, этап продажи, результат кампании, разрешение на контакт, техническое состояние, причину, примечание, даты, менеджера, оффер, выплату, резюме и ссылку на диалог.  
...
**GSH-008.** Целевое время обновления статусных измерений и результата кампании в таблице — не более пяти минут при доступном API Google.

### 6.18. API, webhooks и postback

**INT-001.** API предоставляет компании доступ к клиентам, сообщениям, статусным измерениям, результатам кампаний, кампаниям, офферам и результатам продаж в пределах прав.  
**INT-002.** Контакты можно создавать и обновлять через API.  
**INT-003.** Исходящие события: новый ответ, изменение этапа продажи, разрешения на контакт или технического состояния, результат кампании, квалификация, оффер, передача, блокировка, успешный результат и архивирование.  
...
flowchart TD
    ...
    F -->|Ответ| D
    D --> REF["Отказ от предложения"]
    D -->|Явная просьба не писать| STOP["Стоп-лист"]
    D --> Q["Квалифицирован"]
    Q --> O["Оффер"]
...
- Этап продажи, разрешение на контакт и техническое состояние изменяются независимо; результат хранится отдельно для каждой кампании.
- AI предлагает только разрешённые переходы и действия; backend проверяет и применяет их по каждому измерению отдельно.
- Менеджер может исправить разрешённые значения с обязательной причиной.
- Массовое снятие стоп-листа и изменение подтверждённых блоков требуют специальных прав и двухэтапного подтверждения.
...
- Независимые статусные измерения, результаты кампаний и аудит.
...
- Контакты, дубликаты, независимые статусные измерения и результаты кампаний.
...
- Server-side контрольный барьер Telegram/AI с режимом `запрещено по умолчанию` и реестр решений.
- Docker/Railway.
...
- Память и резюме.
- Классификация intent, этапа продажи, просьбы прекратить контакт и handoff; техническое состояние определяет программа.
- Выбор оффера.
...
- Тестовые сценарии.
- Пороги уверенности.
- Правила независимых статусных измерений, результатов кампаний и handoff.
...
- актуальные условия Telegram API и Telegram Bot Platform для планируемых пользовательских аккаунтов, ботов, данных и AI-операций.

Результат: воспроизводимый прототип, версия реестра совместимости и запись решения по контрольному барьеру. На этом этапе реальные Telegram-сообщения не передаются AI.

**Контрольная точка:** техническая схема воспроизводима, а AI-доступ к реальным Telegram-диалогам остаётся выключенным до отдельного утверждения допустимой модели.
...
**Контрольная точка:** AI проходит эталонные сценарии, не меняет оффер, корректно передаёт исключения и сохраняет память независимо от аккаунта. Подключение к реальным Telegram-диалогам проверяется только после утверждения контрольного барьера для соответствующего канала и сценария.
...
- Тесты server-side барьера: при отсутствии подходящего утверждённого решения реальные Telegram-сообщения не попадают в AI, а AI-отправка отклоняется.
...
- Обычный отказ без просьбы прекратить контакт.
- Явная просьба больше не писать.
- Низкая уверенность.
...
3. При отсутствии подходящего утверждённого решения server-side барьер не допускает передачу реальных Telegram-сообщений в AI и автоматическую AI-отправку.
4. Утверждение барьера фиксирует дату, проверенную редакцию условий, разрешённые каналы, данные, операции, ограничения и ответственного; включаются только перечисленные сценарии.
5. Аккаунт работает через назначенный прокси и восстанавливается после перезапуска.
6. Импортируется не менее 1 000 контактов без потери строк.
7. Дубликаты корректно объединяются или уходят на проверку.
8. Массовые действия применяют фильтр, показывают количество и формируют отчёт.
9. Этап продажи, разрешение на контакт и техническое состояние хранятся независимо; изменение одного не стирает остальные, а `Игнор` остаётся результатом конкретной кампании.
10. Обычный отказ меняет этап продажи, но не добавляет клиента в стоп-лист без явной просьбы прекратить контакт.
11. Стоп/блок не попадает в отправку без отдельного двухэтапного снятия запрета.
12. Цепочка 1–3 сообщений соблюдает расписание, тихие часы, задержки и лимиты.
13. Перезапуск не создаёт повторных сообщений.
14. При блокировке аккаунта необработанные контакты возвращаются в очередь.
15. Активные потерянные диалоги появляются в аварийной очереди.
16. История и память сохраняются после архивирования аккаунта.
17. AI использует выбранный профиль, релевантные знания, память и разрешённые офферы.
18. Внешний поиск соблюдает приоритет источников и сохраняет доказательства.
19. Неподтверждённый кандидат не используется в ответе.
20. AI передаёт сложный диалог с резюме и причиной.
21. После подключения менеджера AI не отправляет автоматически.
22. Партнёрский клик, postback, статус, выплата и валюта связываются с клиентом и оффером.
23. Google Sheets обновляется в пределах пяти минут при доступном API.
24. Аналитика показывает согласованный набор метрик.
25. Все критические действия присутствуют в аудите.
26. Резервная копия успешно восстанавливается в тестовой среде.
...
| Недопустимая модель Telegram/AI | Обязательный контрольный барьер, документированное решение и server-side запрет по умолчанию |
...
MVP строится как модульная платформа для нескольких компаний, но без преждевременной SaaS-сложности. Первый приоритет — надёжная инфраструктура контактов, аккаунтов, прокси, очередей и менеджеров. Второй — управляемый AI-диалог с памятью, знаниями, актуальным поиском и офферами после прохождения обязательного контрольного барьера. Третий — модерируемое накопление опыта, A/B-тесты и аналитическая оптимизация.
